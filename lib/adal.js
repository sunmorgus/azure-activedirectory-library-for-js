...
var AuthenticationContext = (function () {
     ...
     *  @property {string} cacheLocation - Sets browser storage to either 'apim', 'localStorage' or sessionStorage'. Defaults to 'sessionStorage'.
     * @property {string} apimSubscriptionKey - Sets the APIM subscription key for cacheLocation = 'apim'
     * @property {string} apimUrl - Sets the APIM URL for cacheLocation = 'apim'
     * @property {string} apimUserId - Sets the APIM UserId for cacheLocation = 'apim'
  ...
  */
    ...
    */
    ...
    AuthenticationContext.prototype._saveItem = function (key, obj, preserve) {
        ...
        if (this.config && this.config.cacheLocation && this.config.cacheLocation === 'apim') {
            var request = require("sync-request");

            key = key + this.config.apimUserId;

            var body = "";
            if (preserve) {
                var value = this._getItem(key) || '';
                body = value + obj + this.CONSTANTS.CACHE_DELIMETER;
            }
            else {
                body = obj;
            }
            ...
            var response = request('POST', this.config.apimUrl + '/saveItem', options);
    ...
    AuthenticationContext.prototype._getItem = function (key) {
        ...
        if (this.config && this.config.cacheLocation && this.config.cacheLocation === 'apim') {
            ...
            var response = request('GET', this.config.apimUrl + '/getItem', options);
